...
func Get(ctx context.Context, url string, resp any) (err error) {
	return request(ctx, http.MethodGet, url, resp, nil)
}
...
//
// If the request body is not nil, it will set the Content-Type header to "application/json".
func Post(ctx context.Context, url string, resp any, req any) (err error) {
	return request(ctx, http.MethodPost, url, resp, req)
}
